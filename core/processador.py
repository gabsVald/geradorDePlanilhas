...
def f_valido(f):
    """Verifica se o item é válido com base nas regras do JSON."""
...
def is_prensado(r):
    """Verifica se a linha é um gatilho de prensado (via JSON)."""
...
def processar_clipboard(is_teste=False):
    """Função principal que processa os dados do PDM."""
    ...
    for nv_p, info in cons.items():
        ...
        if cam_net and "PLANOS DE CORTE 2026" not in str(cam_net):
            itens_mig, a3_mig = extrair_dados_migracao(cam_net)
            if itens_mig:
            ...
            projetos_duplicados.append(cod_p)
        else: 
            processar_list.append((nv_p, info))
    ...
    if arquivos_para_arquivar and not is_teste:
        ...
        if not dir_antigos.exists(): 
            os.makedirs(dir_antigos)
        for arq_antigo in arquivos_para_arquivar:
            try:
                shutil.move(arq_antigo, dir_antigos / os.path.basename(arq_antigo))
            except Exception as e: 
                print(f"[ERRO ARQUIVAR] {e}")

    # Retorna os resultados para a Interface Gráfica apresentar os avisos
    return {
        "pasta": str(pasta),
        "migrados": arquivos_migrados,
        "repetidos": projetos_duplicados
    }