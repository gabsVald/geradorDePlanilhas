...
class AppIngecon(ctk.CTk):
    ...
    def atualizar_visual_teste(self):
        if self.modo_teste_ativo:
            self.btn_processar.configure(
                fg_color=COR_TESTE, hover_color="#d35400", text="MODO TESTE ATIVO"
            )
        else:
            self.btn_processar.configure(
                fg_color=COR_PRINCIPAL, hover_color=COR_HOVER, text="Colar - Gerar Planilhas"
            )
    ...
    def sucesso_final(self, resultado):
        ...
        # RN-041 — Popup de duplicados (já existem em PLANOS DE CORTE 2026)
        ...
        # RN-040 — Popup informativo de planilhas migradas
