    ...
    def gerar_arquivo_excel(self, pai, blocos, id_proj, qtd_tot, molde, pasta, pai_is_prensado):
        ...
        for b in sorted(blocos, key=lambda x: 0 if x['tipo'] == 'normal' else 1):
            ...
            if bloco_e_prensado:
                max_c, max_f = 0, 0
                for item in b['itens']:
                    v_c_raw = self.limpar(item.get(15, "")) if self.limpar(item.get(15, "")) not in ["", "-", "="] else item.get(8, "")
                    v_l_raw = self.limpar(item.get(16, "")) if self.limpar(item.get(16, "")) not in ["", "-", "="] else item.get(10, "")
                    
                    val_c = self.converter_para_numero(v_c_raw)
                    val_l = self.converter_para_numero(v_l_raw)
                    
                    if isinstance(val_c, int) and val_c > max_c: max_c = val_c
                    if isinstance(val_l, int) and val_l > max_f: max_f = val_l
                
                titulo_bloco = self.limpar(b['prensado_info'][3]) if b['tipo'] == 'prensado' else desc_p
            ...
            for item in b['itens']:
                ...
                # --- CORREÇÃO: SE FOR PRENSADO, ITENS ESPECIAIS NÃO GANHAM +5 ---
                ...
                # -------------------------------------------------------------------------
                # Coluna H (8): Sempre recebe a espessura lida do item original no PDM
                ws.cell(row=r, column=8).value = self.converter_para_numero(item.get(12, ""))
                
                # Coluna L (12): Recebe a descrição montada caso seja prensado, senão recebe o padrão
                if bloco_e_prensado:
    ...
    def core_processamento(self):
        ...
        try:
            ...
            def f_valido(f):
                ...
                # --- CORREÇÃO: LAMINADO FORM LIBERADO, LAMINA PINUS BLOQUEADO ---
                if 'CORTE' in a or any(x in d for x in ["LAMINA MADEIRA", "LAMINA MAD", "LAM MAD", "LAM MADEIRA", "LAMINA PINUS"]) or \
                   any(x in mc for x in ["LAMINA MADEIRA", "LAMINA MAD", "LAM MAD", "LAM MADEIRA", "LAMINA PINUS"]): 
                    return False
